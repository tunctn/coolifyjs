export type StandaloneMongodb = {
  id: number; // bigserial, not null
  uuid: string; // varchar(255), not null
  name: string; // varchar(255), not null
  description: string | null; // varchar(255), nullable
  mongo_conf: string | null; // text, nullable
  mongo_initdb_root_username: string; // text, not null
  mongo_initdb_root_password: string; // text, not null
  mongo_initdb_database: string; // text, not null
